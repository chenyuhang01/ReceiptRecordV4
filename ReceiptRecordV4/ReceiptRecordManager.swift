...
extension ReceiptRecordManager {
    func postMessage(msg: NSNotification.Name) {
        ...
        case .databaseUpdatesPostMessage:
            centre.post(name: msg, object: self.databaseObj)
            break
        case .receiptRecordUpdatesPostMessage:
            centre.post(name: msg, object: self.receiptRecords)
            break
...
extension ReceiptRecordManager {
    ...
    func spawnNewReceiptRecord() -> ReceiptRecord? {
        return ReceiptRecord.generateReceiptRecord()
    }
    ...
    func removeReceiptRecord(record: ReceiptRecord) {
        self.receiptRecords?.removeExistingRecord(receiptRecord: record)
        self.postMessage(msg: .receiptRecordUpdatesPostMessage)
    }
    
    // Get the particular record, if not nil
    func getReceiptRecord(index: Int) -> ReceiptRecord? {
        
        if let receiptRecords = self.receiptRecords {
            if index < receiptRecords.getCount() {
                return receiptRecords[index]
            }
        }
        return nil
    }
